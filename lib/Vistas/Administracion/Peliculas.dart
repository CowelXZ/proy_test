    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          peliculasFiltradas =
              List.from(peliculas); // <- ACTUALIZA TAMBIÉN LA FILTRADA
        });
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 5),
                Text(
                  'Idiomas: ${Pelicula['Idioma'] ?? 'Desconocido'}',
                  style: const TextStyle(fontSize: 14),
                ),
...
/*
void _eliminarPelicula(int ID_Pelicula) async {
  try {
    final response = await http.delete(
      Uri.parse('http://localhost:3000/deleteMovie/$ID_Pelicula'),
    );
    ...
    if (response.statusCode == 200) {
      ...
      setState(() {
        peliculas.removeWhere((pelicula) => pelicula['ID_Pelicula'] == ID_Pelicula);
      });
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
