...
class UserController {
  ...
  Future<List<User>> fetchUsuarios(String dropdownValue) async {
    final url = Uri.parse(
        '$baseUrl/getUsers?orderBy=Departamento&departamento=$dropdownValue');
    try {
...
// ✅ Guardar Usuario
Future<void> saveUser(BuildContext context, User user, String contrasena, String confirmarContrasena) async {
  try {
    // Verificar que todos los campos estén llenos
    if ([user.nombreCompleto, user.apellidos, user.telefono, user.usuario, user.cumpleanos, user.rfc, user.departamento, contrasena, confirmarContrasena].any((element) => element.isEmpty)) {
    ...
    if (!telefonoRegex.hasMatch(user.telefono)) {
      mostrarMensaje(context, "El teléfono debe contener 10 dígitos numéricos.");
      return;
    }
    ...
    if (contrasena != confirmarContrasena) {
      mostrarMensaje(context, "Las contraseñas no coinciden. Inténtalo de nuevo.");
      return;
    }
    ...
    if (response.statusCode == 201) {
      mostrarMensaje(context, "Usuario guardado con éxito", color: Colors.green);
